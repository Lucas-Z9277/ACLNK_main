...
class ACLNK_ModelConfig(DotDict):
    '''
    configuration of the ACLNK
    '''
    ...
    def __init__(self, loc_size=None, tim_size=None, uid_size=None, tim_emb_size=None, loc_emb_size=None,
        ...
        # self.aug_friend_vertor = aug_friend_vertor


class ACLNK(nn.Module):
    def __init__(self, config):
        super(ACLNK, self).__init__()
        ...
        # self.aug_friend_vertor = config['aug_friend_vertor']
        ...
        if self.downstream == 'TUL':
            ...
            raise ValueError('downstream should in [TUL, NPR]!')
    ...
    def generate_adv(self, Anchor_hiddens, lm_labels):
        ...
        perturbed_Anc = Anchor_hiddens + self.neg_eps * dec_grad.detach()###参数
    ...
    def forward(self, batch, mode='test', adv=1, downstream='NPR', aug_friend_vector=None):
        loc = batch.X_all_loc
        tim = batch.X_all_tim
        user = batch.X_users
        ...
        if mode == 'train' and adv == 1:
            loc_noise = torch.normal(self.loc_noise_mean, self.loc_noise_sigma, loc_emb.shape).to(loc_emb.device)####初始扰动参数
        ...
        if mode == 'train' and adv == 1:
            ...
            cos = nn.CosineSimilarity(dim=-1)
            cont_crit = nn.CrossEntropyLoss()
            sim_matrix = cos(avg_STNPos.unsqueeze(1), avg_Anchor.unsqueeze(0))
            if downstream == 'NPR':
            ...
            # avg_adv_imposter = torch.zeros(adv_imposter.size()).to('cuda')###消融实验
            ...
            ###aug_friend##需要调整sizes
            adv_aug_friend = aug_friend_vector
            ###RICK##
            alpha = 0.9###原设置0.8
            adv_RICK = adv_imposter * (1-alpha) + adv_disTarget * alpha
            avg_adv_RICK = self.projection(adv_RICK)
            pos_sim_RICK = cos(avg_adv_RICK, avg_Anchor).unsqueeze(-1)###avg_adv_disTarget,unsqueeze(1)或(-1)没区别size(4,1)

            pos_sim = cos(avg_STNPos, avg_adv_disTarget).unsqueeze(-1)  # [b,1]#####

            avg_adv_disTarget_tensor = F.normalize(avg_adv_disTarget, p=2, dim=1)
            adv_aug_friend_tensor = F.normalize(adv_aug_friend, p=2, dim=1)###原本
            # adv_aug_friend_tensor = adv_aug_friend.zero_()###消融实验


            pos_friend_sim = torch.mm(avg_adv_disTarget_tensor, adv_aug_friend_tensor.transpose(0, 1))###avg_adv_disTarget_tensor、anchor、ST
            # # 求每一行的均值，维度变为(4,)
            # pos_friend_sim_mean = pos_friend_sim.mean(dim=1)
            # 找出每行的最大值和最小值
            max_val, _ = torch.max(pos_friend_sim, dim=1)
            min_val, _ = torch.min(pos_friend_sim, dim=1)
            # pos_friend_sim_mean = (max_val + min_val) / 2
            pos_friend_sim_mean = max_val
            # 增加一个维度，维度变为(4, 1)
            pos_friend_sim_mean_resized = pos_friend_sim_mean.unsqueeze(1)
            # pos_friend_sim_log = torch.log(pos_friend_sim_mean_resized)###需要修改
            # pos_friend_sim_log_mean = (pos_friend_sim_mean_resized - pos_friend_sim_mean_resized.min()) / (pos_friend_sim_mean_resized.max() - pos_friend_sim_mean_resized.min())

            logits = torch.cat([sim_matrix, adv_sim], 1) / self.tau#########logits
            identity = torch.eye(batch_size, device=final_out.device)
            pos_sim = identity * pos_sim
            neg_sim = sim_matrix.masked_fill(identity == 1, 0)
            pos_friend_sim_result = identity * pos_friend_sim_mean_resized
            new_sim_matrix = pos_sim + neg_sim + pos_friend_sim_result###原本
            # new_sim_matrix = pos_sim + neg_sim ###消融实验
            # new_sim_matrix = pos_friend_sim_result##消融实验NSKE
            pos_sim_RICK = identity * pos_sim_RICK
            pos_sim_RICK_matrix = pos_sim_RICK
            pos_RICK_logits = torch.cat([pos_sim_RICK_matrix, adv_sim], 1)

            new_logits = torch.cat([new_sim_matrix, adv_sim], 1)#########new_logits
            # new_logits = torch.cat([new_sim_matrix, torch.zeros_like(adv_sim)], 1)###消融实验NSKE
            ...
            ####RICK_Loss##
            RICK_Loss = cont_crit(pos_RICK_logits, labels)


            cont_loss_final = (cont_loss + new_cont_loss + RICK_Loss) / 3 ###原本
        ...
        if mode == 'train' and adv == 1:
            return s_loss_score, cont_loss_final, top_k_pred, indice###
        else:
