...
import preprocess.load_data_for_ACLNK as preprocess 
from model.ACLNK import *
...
import time
from test_friend_input import self_friend_neg2, dict_convert, dict_seq_equal_padding
import Gan_Training as gt
import Transformer.transformer as all_module
import torch.optim as optim
import tools.utils as ut
from model import AutoEnc4Rec_cross as Model
...
parser.add_argument("--config", default='config/ACLNK_gow_NPR.conf', type=str,
...
#     def __init__(self, seq, len, user):
#         self.seq = seq
#         self.len = len
#         self.user = user
#
#     def __getitem__(self):
#         return self.seq, self.len, self.user
...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return {
            'seq': torch.tensor(self.data['seq'][idx]),
            'len': torch.tensor(self.data['len'][idx]),
            'user': torch.tensor(self.data['user'][idx])
        }

parser = argparse.ArgumentParser()
parser.add_argument("--lr", type=float, default=0.001, help="learning rate")
parser.add_argument("--date", type=str, default="sas_org", help="labeling")
parser.add_argument("--d_model", type=int, default=1024, help="predictive factors numbers in the model")
...
parser.add_argument("--cross", type=str, default="True", help="whether to use sas")
...
args_gan = parser.parse_args()
args_gan.fix_enc = str2bool_par(args_gan.fix_enc)
param_gan = param_c.get_param(args_gan)####config_auto4rec
...
if downstream in ["TUL"]:
    args_gan.d_model = 1024
else:
    args_gan.d_model = 2048
...
if __name__ == '__main__':
    ...
    ###First, Adversarial generalizing representation module computing
    # self_input, friend_input, neg_input, max_len_all = self_friend_neg2()
    #
    # self_input_pad = dict_seq_equal_padding(self_input, max_len_all)
    # friend_input_pad = dict_seq_equal_padding(friend_input, max_len_all)
    # # neg_input_pad = dict_seq_equal_padding(neg_input, max_len_all)
    #
    # self_input_convert = dict_convert(self_input_pad)
    # friend_input_convert = dict_convert(friend_input_pad)
    # # neg_input_convert = dict_convert(neg_input_pad)
    #
    #
    # self_input_tensor = Dataloader.dataloader_gen(self_input_convert, param_gan, max_len_all, train=True,
    #                                               seq_len=None, wf=None, domain="a", n_w=1)
    # friend_input_tensor = Dataloader.dataloader_gen(friend_input_convert, param_gan, max_len_all,
    #                                                 train=True, seq_len=None, wf=None, domain="b", n_w=1)
    #
    # dl_train_aug = [self_input_tensor, friend_input_tensor]
    #
    ...
    # # dis
    # netD = ut.Discriminator(param_gan.d_model, 1, param_gan.dis_dim).to(torch.float32).to(device)
    # if torch.cuda.device_count() > 1:
    #     netD = nn.DataParallel(netD)
    # opt_dis = optim.Adam(netD.parameters(), lr=0.0001, betas=(0.5, 0.9))
    #
    # aug_friend_vector = gt.train_gan_all(enc_model, netD, dl_train_aug, opt_dis, opt_gen,
    #                                      device, param_gan, param_gan.training_steps_tune, max_len_all, domain="a",
    #                                      overlap=False)
    #
    # torch.save(aug_friend_vector, dataset_name + 'gow_tul_aug_friend_vector.pt')

    aug_friend_vector = torch.load('data/new_datasets/' + dataset_name + downstream +'aug_friend_vector.pt')
    aug_friend_vector = aug_friend_vector.detach()
    print('aug_friend_vector load success！')
    # Data
    ...
    print('Loading data...')
    data_train, data_val, data_test, feature_category, feature_lat, feature_lng, latN, lngN, category_cnt = preprocess.load_dataset_for_ACLNK(
    ...
    dl_train = torch.utils.data.DataLoader(data_train, batch_size=train_batch, shuffle=True, collate_fn=collate)
    ...
    general_config = ACLNK_ModelConfig(loc_size=int(data_train.venue_cnt), tim_size=tim_size,
    ...
    # Define model
    model = ACLNK(general_config).to(device)
    ...
    if mode == 'train':
        ...
        for epoch in range(0, max_epochs):
            ...
            for input in dl_train:
                ...
                if adv == 1:
                    s_loss_score, cont_loss, top_k_pred, indice = model(input, mode='train', adv=adv, downstream=downstream, aug_friend_vector=aug_friend_vector)##修改，添加friend_aug
                    loss_total = (1 - self_weight) * s_loss_score + cont_loss * self_weight
                else:
                    s_loss_score, top_k_pred, indice = model(input, mode='train', adv=adv, downstream=downstream, aug_friend_vector=aug_friend_vector)
                ...
                if downstream == 'NPR':
                    ...
                    raise ValueError('downstream is not in [NPR, TUL]')
            ...
            model.eval()
            with torch.no_grad():
                all_loss_s_val, hit_ratio_val, mrr_val = get_s_baselines_total_loss_s_for_ACLNK_DOWN(dl_val, model, downstream=downstream)
    ...
    model.eval()
    with torch.no_grad():
        train_all_loss_s, train_hit_ratio, train_mrr = get_s_baselines_total_loss_s_for_ACLNK_DOWN(dl_train, model, downstream=downstream)
        val_all_loss_s, val_hit_ratio, val_mrr = get_s_baselines_total_loss_s_for_ACLNK_DOWN(dl_val, model, downstream=downstream)
        test_all_loss_s, test_hit_ratio, test_mrr = get_s_baselines_total_loss_s_for_ACLNK_DOWN(dl_test, model, downstream=downstream)
        ...
        output_text = 'Dataset\t loss\t hit_1\t hit_3\t hit_5\t hit_7\t hit_10\t hit_15\t hit_20\t MRR\t\n' \
                      'Train:\t %.4f\t %.4f\t %.4f\t %.4f\t %.4f\t %.4f\t %.4f\t %.4f\t %.4f\t\n' % (
                          train_all_loss_s, train_hit_ratio[0], train_hit_ratio[2], train_hit_ratio[4], train_hit_ratio[6],
                          train_hit_ratio[9], train_hit_ratio[14], train_hit_ratio[19], train_mrr) + \
                      'Val:\t %.4f\t %.4f\t %.4f\t %.4f\t %.4f\t %.4f\t %.4f\t %.4f\t %.4f\t\n' % (
                          val_all_loss_s, val_hit_ratio[0], val_hit_ratio[2], val_hit_ratio[4], val_hit_ratio[6],
                          val_hit_ratio[9],
                          val_hit_ratio[14], val_hit_ratio[19], val_mrr) + \
                      'Test:\t %.4f\t %.4f\t %.4f\t %.4f\t %.4f\t %.4f\t %.4f\t %.4f\t %.4f\t\n' % (
                          test_all_loss_s, test_hit_ratio[0], test_hit_ratio[2], test_hit_ratio[4], test_hit_ratio[6],
                          test_hit_ratio[9], test_hit_ratio[14], test_hit_ratio[19], test_mrr)
