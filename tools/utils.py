...
def get_next_batch(dataloader_iterator, device):
    ...
    :return:
    """
    seqs, seq_len, user = next(dataloader_iterator)
    seqs_stack = torch.stack(seqs).type(torch.int32)
    seqs_stack, seq_len, user = seqs_stack.to(device), seq_len.to(device), user.to(device)
    return seqs_stack, seq_len, user
