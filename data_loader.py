...
class pickle_loader_eval(data.Dataset):
    ...
    """
    def __init__(self, file, param, num_n, seq_len=None, wf=None, domain="a"):
        self.seq = []  # need padding.##目前不需要
        self.len = []
        self.user = []
        ...
        if self.seq_len:  # select user with behavior length fall in range "seq_len"
            for i in range(len(data_tmp["val"])):
                if self.seq_len[1] > len(data_tmp["seq"][i]) > self.seq_len[0]:
                    ...
                    self.len.append(data_tmp["len"][i])
                    self.user.append(data_tmp["user"][i])
        else:
            self.seq.extend(data_tmp["seq"])
            self.len.extend(data_tmp["len"])
            self.user.extend(data_tmp["user"])
    ...
    def __getitem__(self, index):
        ...
        """
        seq, len, user = self.seq[index], self.len[index], self.user[index]
        # seq, user = self.seq[index], self.user[index]
        eval_enc_in, dec_in, test_enc_in, t_dec_in = test_seq_gen(seq, self.enc_len, self.dec_len, self.eos, len)
        ...
        return (eval_enc_in, dec_in, len), (test_enc_in, t_dec_in, user), n_items_f, n_items_r
...
class pickle_loader(data.Dataset):
    ...
    def __init__(self, file, param, num_n, seq_len=None, wf=None, domain="a"):
        ...
        data_tmp = file  ####load data
        if self.seq_len:  # select user with behavior length fall in range "seq_len"
            for i in range(len(data_tmp["len"])):
                if self.seq_len[1] > len(data_tmp["seq"][i]) > self.seq_len[0]:
                    self.seq.append(data_tmp["seq"][i])
                    self.len.append(data_tmp["len"][i])
                    self.user.append(data_tmp["user"][i])
...
def dataloader_gen(all_files, param, num_batch, train=True, seq_len=None,
                   wf=None, domain="a", n_w=1):
    """
    Normal dataloader
    :param n_w:             number of workers数据加载的子进程数量
    :param domain:
    :param wf:
    :param rec:             rec=True, determine the seq len to the rec model. (dec_in dec_out)
    :param param:
    ...
    """
    if not train:
        dataset = pickle_loader_eval(all_files, param, num_batch, seq_len=seq_len, wf=wf, domain=domain)
        dataloader = torch.utils.data.DataLoader(dataset, batch_size=num_batch,
                                                 shuffle=train, num_workers=n_w,
                                                 drop_last=True, collate_fn=my_collate)
    else:
        dataset = pickle_loader(all_files, param, num_batch, seq_len=seq_len, wf=wf, domain=domain)
        dataloader = torch.utils.data.DataLoader(dataset, batch_size=num_batch,
